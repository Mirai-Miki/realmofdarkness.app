...
- Adhere strictly to the project's ESLint (`eslint.config.mjs`) and Prettier (`.prettierrc.json`) configurations. Generated code must pass all type and lint checks without warnings or formatting violations.
...
## 3. Monorepo Package Topology & Boundaries
...
- Do not include apologies, explanations of your intent, or conversational filler.
- Output explicit, modular, typed code. Defer styling corrections entirely to the repository's native linter and formatter tasks.
