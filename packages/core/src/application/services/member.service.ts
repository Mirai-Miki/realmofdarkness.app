...
import type { IMemberRepository } from "../../ports/member.repository.interface.js";
import type { ISupporterRepository } from "../../ports/supporter.repository.interface.js";
import type { IUserRepository } from "../../ports/user.repository.interface.js";
import { Member } from "../../domain/entities/member.entity.js";
...
export class MemberService {
  ...
  async create(
    ...
    if (!userExists) {
      logger.debug(`Skipping member add: User ${userId} not registered`);
      return null;
    }
    ...
    logger.info(`Created member record`, {
      fields: { userId, guildId },
    });
  ...
  async update(
    ...
    logger.debug(`Synced member data`, { fields: { userId, guildId } });
  }
  ...
  async delete(guildId: Snowflake, userId: Snowflake): Promise<void> {
    logger.info(`Deleting member from guild ${guildId}`, {
      fields: { userId, guildId },
    });
  ...
  async deleteByGuild(guildId: Snowflake): Promise<void> {
    logger.info(`Deleting all members from guild ${guildId}`, {
      fields: { guildId },
    });
  ...
  async addBoost(guildId: Snowflake, userId: Snowflake): Promise<void> {
    try {
      ...
      logger.debug(`Checking boost availability for user ${userId}`, {
        fields: {
      ...
      logger.info(`Boost added for user ${userId} in guild ${guildId}`);
    } catch (error) {
  ...
  async removeBoost(guildId: Snowflake, userId: Snowflake): Promise<void> {
    try {
      ...
      logger.info(`Boost removed for user ${userId} in guild ${guildId}`);
    } catch (error) {
